...
//var secrettext = "UserInfo";


//var testvalue = cookieSignature.sign(secrettext,"this is a test");
...
router.get('/:anything',function(req,res,next){
	...
	if(cookieparsing)
	{
		...
		{
			if(testing[1] == "cleared")
			{
				next();
			}else	
			{
				var clearedcookie = "cleared"	
				var dateRemove = new Date(0).toUTCString();
				res.setHeader('Set-Cookie',cookie.serialize('UserInfo',clearedcookie,{
					maxAge:0,
					expire: dateRemove
				}));
...
router.post('/register', function(req,res)
{
	...
	var usercookie=cookie.sign(username,usernames)+'-'+DateKey;
